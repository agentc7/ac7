 ...
 * calls. The ac7 broker is the source of truth for everything
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification(payload.title, {
      ...
      // individual-contributors don't miss a `warning`/`error`/`critical` broadcast
...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(
    (async () => {
      ...
      // one — individual-contributors running the PWA usually already have it open.
