...
function makeDevOnSuccess(): () => Promise<() => Promise<void>> {
  return async () => {
    const child: ChildProcess = spawn(
      process.execPath,
      ['--env-file-if-exists=../../.env', './dist/index.js', '--config-path', '../../ac7.json'],
      { stdio: 'inherit' },
    );
    return async () => {
...
export default defineConfig([
  ...
  // Bin entry (consumed by `ac7-server`). Watch-mode onSuccess lives
  ...
  // Second bin: `ac7-connect-platform` — drives the server side of
  // the registration device flow that pairs this ac7 with a hosted
