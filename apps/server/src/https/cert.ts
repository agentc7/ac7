/**
 * Self-signed TLS certificate generation for ac7.
 *
 * Wraps the `selfsigned` npm package (which is built on node-forge) with
 * ac7's fixed defaults: RSA 2048 + SHA-256, 365-day validity,
...
export async function generateSelfSignedCert(
  ...
  const pems = await generate([{ name: 'commonName', value: 'ac7' }], {
