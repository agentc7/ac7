...
export function createApp(options: AppOptions): CreatedApp {
  ...
  //
  // Bridges an ac7 deployment into a hosted control plane (the
  ...
  // even when the ac7 web bundle isn't served from the same host, and
  // so we don't have to push another route into the shared
  // @agentc7/web-shell package (which would leak into the
  ...
  <script>
    (function () {
      ...
      function renderSignedOut() {
        root.innerHTML =
          '<h1>Sign in first</h1>' +
          '<p>Sign into this ac7 server as the member you want to bind to the platform.</p>' +
