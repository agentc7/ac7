 ...
 * at rest.
 *
 * The team config file (`ac7.json`) stores bearer tokens as
 ...
 * exfiltration of `ac7.json` leaked every slot's web-login
 ...
 *        (a) `AC7_KEK` env var — base64 / base64url of exactly 32 bytes
 *        (b) a persisted key file at `<configDir>/ac7-kek.bin`
 *            (`0o600`). Auto-generated on first resolve if missing.
 *      Either source alone is enough; individual-contributors who want the KEK
 *      managed by their OS keychain / secret manager inject it via
 *      `AC7_KEK`. Zero-config self-host users get the file-based
 *      default.
 *
 ...
 *     default key location is `<configDir>/ac7-kek.bin`, and we
 *     recommend individual-contributors chmod the containing directory (`0o700`)
...
const KEK_ENV_VAR = 'AC7_KEK';
const KEK_FILE_NAME = 'ac7-kek.bin';
const KEK_BYTES = 32;
const IV_BYTES = 12;
 ...
 *   to pin a specific KEK via `AC7_KEK`.
 */
...
export function encryptField(plaintext: string | null | undefined, kek: Buffer): string | null {
  ...
  if (plaintext.startsWith(ENCRYPTED_FIELD_PREFIX)) {
    // Already encrypted — idempotent. Lets `writeTeamConfig` be
