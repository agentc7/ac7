...
export interface TeamConfig {
   ...
   * a JWKS-backed verifier. Null when the deployment doesn't
   * federate with any external issuer.
   */
  jwt: JwtConfig | null;
  migrated: number;
}
...
export function loadTeamConfigFromFile(path: string): TeamConfig {
  ...
  // Check for a platform overlay sibling (`<path>.platform.json` for
  // `<path>` ending in `.json`, else `<path>.platform.json`). The
  // overlay is what `ac7-connect-platform` writes to — it carries
  // ONLY the jwt block, and takes precedence over whatever the user
  // has in the primary config. Keeping it separate means the user's
  // config.json stays untouched by the CLI and the overlay can be
  // deleted to disable platform federation cleanly.
  const overlayJwt = loadPlatformOverlay(path);
...
/**
 * Read the platform overlay if present and validate its shape.
 * Returns null when the file is missing (common path — most
 * deployments don't connect to a hosted platform). A malformed
 * overlay is a hard error so a bad write from `ac7-connect-platform`
 * fails loud instead of the server silently running with stale JWT
 * config.
 */
function loadPlatformOverlay(primaryConfigPath: string): JwtConfig | null {
  const overlayPath = platformOverlayPathFor(primaryConfigPath);
  let raw: string;
  try {
    ...
    throw new MemberLoadError(
      `failed to read platform overlay at ${overlayPath}: ${(err as Error).message}`,
    );
  }
  let parsed: unknown;
  try {
    ...
    throw new MemberLoadError(
      `platform overlay at ${overlayPath} is not valid JSON: ${(err as Error).message}`,
    );
  }
  const result = PlatformOverlaySchema.safeParse(parsed);
  if (!result.success) {
    const issues = result.error.issues
      .map(
        (issue: { path: PropertyKey[]; message: string }) =>
      ...
      .join('\n');
    throw new MemberLoadError(`platform overlay at ${overlayPath} is invalid:\n${issues}`);
  }
 ...
 * Future platform-managed settings (e.g. quota reporting, telemetry
 * opt-in) would add fields here; leaving the door open but not
 * opening it unnecessarily.
 */
const PlatformOverlaySchema = z.object({
  jwt: JwtConfigSchema,
});
