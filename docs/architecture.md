...
server is authoritative about a team's directive, roles, slots,
authority levels, objectives, and captured LLM traces. An individual contributor
runs `ac7 claude-code` as a **runner**, the runner spawns `claude` as
a child, and inside that session the agent speaks MCP over stdio to a
thin `ac7 mcp-bridge` relay — which hands every request back to the
...
```
            ┌─ humans ─┐                    ┌─ runners ──────────┐
            │ browser  │                    │ individual contributor's         │
            └────┬─────┘                    │ ac7 claude-code    │
...
- **Machine plane** — `Authorization: Bearer ac7_…`. Tokens live in
  the team config (SHA-256 hashed on disk) and authenticate the
  runner subprocess.
- **Human plane** — `ac7_session` cookie minted after a TOTP login.
...
`/history`, `/objectives/*`, `/agents/:name/activity`) never
...
| **manager** | Create objectives (as originator), cancel objectives they originated, post to discussion threads they're members of |
...
The individual contributor authority is the default; directors are set explicitly
in the team config. Authority is checked server-side on every
mutating endpoint; there's no client-side gate you can skip.

## The runner / bridge process tree

The most important piece of ac7's architecture is how the
individual contributor's CLI process relates to the agent it's driving:

```
                   individual contributor terminal
                          │
                          ▼
           ┌──────────────────────────────────┐
           │   ac7 claude-code                │
...
have it talk HTTP directly to the broker — is what ac7 used to
...
Objectives are ac7's structured work primitive — push-assigned,
...
- **Threaded discussion**: each objective gets its own thread at
  `obj:<id>`. Originator + assignee + directors + explicit watchers
...
```
   ...
   └───────┬───────┘
           ...
           ▼
   ┌───────────────┐
   │ ActivityUpload│  batched POST /agents/:name/activity.
...
Directors view traces in the web UI's **TracePanel** on each
objective's detail page. Non-directors get a 403 from
...
```
  ...
  ┌──────────────┐   ┌───────────┐   ┌───────────┐   ┌─────────────┐
  │  ac7 claude- │   │  TS SDK   │   │  ac7 CLI  │   │  web UI     │
...
- **Name** — the slot's identity in the net. What `message.from`
  ...
  already has.
- **Role** — a key into the team's roles map, giving the slot
  role-specific instructions in its briefing. Cosmetic for access
  control; **authority** gates permissions, not role.
- **Authority** — `director | manager | individual contributor`. The real
...
The broker enforces `agentId === slot.name` on subscribe: a slot
can only act on its own agent. DMs are addressed by name;
...
5. Claude Code (via the `claude/channel` capability the bridge
   ...
   `<channel source="ac7" thread="dm" from="NAME">body</channel>`
   tag and injects it into the running model's context.
6. The model wakes and reacts in real time — no user prompt, no
   polling.
7. Simultaneously, the individual contributor's phone buzzes with an OS
...
1. Director or manager creates an objective via `ac7 objectives
...
6. On terminal transition (`done` or `cancelled`), the store emits
   ...
   streamed up.
7. A director opens the objective in the web UI. The TracePanel
   queries `GET /agents/<assignee>/activity?from=<createdAt>
   &to=<completedAt>&kind=llm_exchange`, which 200s only for
   directors. The panel renders model, usage, messages, and
