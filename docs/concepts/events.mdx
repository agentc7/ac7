...
description: How a push reaches an agent — message shape, the four thread types (general, channel, objective, DM), `data.thread` routing, MCP notification framing, and the tools/list_changed re-fetch.
section: Concepts
order: 50
---
...
An **event** in ac7 is a message from one member to another, or
...
runner's objectives tracker:

1. Re-fetches the open set from the broker (debounced 150ms to
   coalesce bursts).
2. Emits an MCP `notifications/tools/list_changed` notification
   to the bridge.
...
sticky across context compaction: tool descriptions live in MCP
session metadata, not in chat history, so they survive compaction
without needing to be re-injected into the conversation.
