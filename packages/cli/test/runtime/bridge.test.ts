 ...
 *
 *   1. Boot a fake ac7 broker on a random localhost port
 ...
 *   3. Spawn `ac7 mcp-bridge` as a subprocess with `AC7_RUNNER_SOCKET`
 ...
 *          capability declared
 *        - `tools/list` returns the full 13-tool surface (director
...
describeIfBuilt('runner + bridge end-to-end', () => {
  ...
  beforeAll(async () => {
    ...
    proc = spawn(process.execPath, [CLI_BINARY, 'mcp-bridge'], {
      env: {
        ...process.env,
        AC7_RUNNER_SOCKET: runner.socketPath,
      },
      stdio: 'pipe',
    });
  ...
  it('completes MCP initialize handshake and declares claude/channel capability', async () => {
    ...
    expect(result.serverInfo.name).toBe('ac7');
