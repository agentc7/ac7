/**
 * `ac7 claude-code` end-to-end smoke test.
 *
 ...
 *
 *   - The runner starts, .mcp.json gets written with a `ac7`
 *     entry pointing at the runner's IPC socket.
 *   - The fake claude "runs" (a bash script that spawns the real
 *     `ac7 mcp-bridge` via the .mcp.json it finds on disk, sends a
 ...
 * works end-to-end from an individual-contributor-level entry point.
 *
...
describeIfBuilt('ac7 claude-code end-to-end', () => {
  ...
  beforeEach(() => {
    sandbox = mkdtempSync(join(tmpdir(), 'ac7-claude-smoke-'));
    transcriptPath = join(sandbox, 'claude-transcript.txt');

    // Fake claude: a bash script that spawns the real `ac7 mcp-bridge`
...
const entry = cfg.mcpServers.ac7;
if (!entry) { console.error('fake-claude: missing ac7 entry'); process.exit(2); }
  ...
  it('starts a runner, wraps fake claude, and restores .mcp.json on exit', async () => {
    ...
    try {
      const exitCode = await runClaudeCodeCommand({
        ...
        // this context. In real individual-contributor use, argv[1] is always the
