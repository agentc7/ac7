/**
 * `ac7 claude-code --doctor` — preflight check for trace capture.
 *
 ...
 *      crypto-runtime issues before the first `ac7 claude-code`
 *
 * The doctor never reaches out to a broker or spawns an agent; it's
 * a local check the individual-contributor runs before their first `ac7 claude-code`
 * invocation.
 *
 ...
 * process's env — that means either the individual-contributor is running doctor
 * inside an `--unsafe-tls` session, or they've set the env var
 * manually (which the runner used to do unconditionally and no
 * longer does). Either way it's a posture the individual-contributor should see
  ...
  if (envVal === '0') {
    return {
      ...
      detail:
        ...
        'The `ac7 claude-code` runner no longer sets this by default. ' +
...
async function checkTmpdir(): Promise<DoctorCheck> {
  const dir = tmpdir();
  const probePath = join(dir, `ac7-doctor-${randomBytes(4).toString('hex')}`);
  try {
 ...
 * Node crypto module, or our own code — individual-contributor should see an
 * actionable error instead of a generic "trace host failed to
 * start" at `ac7 claude-code` time.
 */
