/**
 * IPC protocol between the ac7 runner (parent of the agent) and the
 * `ac7 mcp-bridge` relay (child of the agent, spawned via .mcp.json).
 *
...
export function defaultSocketPath(pid: number = process.pid): string {
  ...
  return `${tmp}/.ac7-runner-${pid}.sock`;
}
 ...
 */
export const RUNNER_SOCKET_ENV = 'AC7_RUNNER_SOCKET';
