/**
 * `ac7 mcp-bridge` — the stdio MCP server agents spawn via `.mcp.json`.
 *
 * This is a **thin relay** that owns nothing except its two endpoints:
 *
 *   stdio <──── MCP JSON-RPC ────>  agent (Claude Code, etc.)
 *   IPC   <──── ac7 IPC frames ──>  runner (the individual-contributor's `ac7 claude-code` process)
 *
 * It doesn't talk to the ac7 broker, doesn't hold a briefing, doesn't
 ...
 * The bridge's job:
 *
 *   1. Read `AC7_RUNNER_SOCKET` from env; refuse to start without it
 ...
 * socket drops. Called from the `ac7 mcp-bridge` CLI verb.
 */
export async function runBridge(): Promise<void> {
  ...
  if (!socketPath || socketPath.length === 0) {
    throw new BridgeStartupError(
      `${RUNNER_SOCKET_ENV} is required — the bridge must be spawned by a ac7 runner ` +
        `(try: ac7 claude-code -- <agent-command>)`,
    );
  }
