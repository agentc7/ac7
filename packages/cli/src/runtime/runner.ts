/**
 * The ac7 runner — the parent process that owns a ac7 session.
 *
 * The runner holds all the heavyweight state for a single agent run:
 *
 *   - the broker `Client` (authenticated to the ac7 server)
 *   - the cached `BriefingResponse` (name, role, authority, team,
 *     directive, initial open objectives)
...
export interface RunnerOptions {
   ...
   * `null`. Default: tracing enabled. `ac7 claude-code --no-trace`
   * sets this to `true`.
   */
  noTrace?: boolean;
  /**
...
export interface RunnerHandle {
   ...
   * `ac7 claude-code` reads this to know whether to bake `ALL_PROXY`
...
export async function startRunner(options: RunnerOptions): Promise<RunnerHandle> {
  ...
  try {
    ...
    // `ac7 serve` isn't running, or `--url` is pointing somewhere else.
    ...
    // mislead the individual-contributor with a "start your broker" hint when the
    ...
    const hint = looksLikeConnectFailure
      ? `\n  hint: is \`ac7 serve\` running at ${options.url}? ` +
