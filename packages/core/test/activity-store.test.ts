import type { ActivityEvent, ActivityKind } from '@agentc7/sdk/types';
...
function opaque(ts: number, host = 'example.com'): ActivityEvent {
  return {
    kind: 'opaque_http',
    ts,
    duration: 50,
...
describe('InMemoryActivityStore.list', () => {
  ...
  it('filters by kind', () => {
    const s = store();
    s.append('engineer-1', [llm(100), opaque(150), objectiveOpen(200)]);
    const opaqueRows = s.list({ memberName: 'engineer-1', kinds: ['opaque_http'] });
