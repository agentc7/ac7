...
describe('InMemoryActivityStore.append', () => {
  it('returns rows with monotonic ids and the injected createdAt', () => {
    ...
    expect(rows[0]?.slotName).toBe('ALPHA-1');
...
describe('InMemoryActivityStore.list', () => {
  it('returns newest-first', () => {
    ...
    const rows = s.list({ slotName: 'ALPHA-1' });
  ...
  it('filters by kind', () => {
    ...
    const opaqueRows = s.list({ slotName: 'ALPHA-1', kinds: ['opaque_http'] });
  ...
  it('filters by from/to inclusive', () => {
    ...
    const rows = s.list({ slotName: 'ALPHA-1', from: 200, to: 300 });
  ...
  it('honors limit', () => {
    ...
    const rows = s.list({ slotName: 'ALPHA-1', limit: 5 });
  ...
  it('clamps limit to maxLimit', () => {
    ...
    s.append('ALPHA-1', events);
    const rows = s.list({ slotName: 'ALPHA-1', limit: 999 });
    expect(rows).toHaveLength(10);
  });

  it('returns empty for unknown slot', () => {
    const s = store();
    expect(s.list({ slotName: 'NOBODY' })).toEqual([]);
  });
  ...
  it('isolates slots from each other', () => {
    ...
    expect(s.list({ slotName: 'ALPHA-1' })).toHaveLength(1);
    expect(s.list({ slotName: 'BRAVO-1' })).toHaveLength(2);
  });
});
...
describe('InMemoryActivityStore.prune', () => {
  ...
  it('cutoff after all rows drains the slot bucket entirely', () => {
    ...
    expect(s.size()).toBe(0);
    expect(s.list({ slotName: 'ALPHA-1' })).toEqual([]);
  });
...
describe('InMemoryActivityStore.subscribe', () => {
  ...
  it('row appears in a synchronous list() from inside a listener', () => {
    ...
    s.subscribe('ALPHA-1', () => {
      seenCountAtFireTime.push(s.list({ slotName: 'ALPHA-1' }).length);
    });
    s.append('ALPHA-1', [llm(100), llm(200)]);
