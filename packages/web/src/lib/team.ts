 ...
 * shaped to accommodate multi-team without a data-model change: when
 * SaaS ships N teams per user, the session payload grows a
 * `teams: Team[]` field and `activeTeamId` becomes a signal of its
 * own. Consumers of `currentTeam` don't need to change.
 */
