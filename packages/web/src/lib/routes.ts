 ...
 *
 * Multi-team: routes accept an optional `/t/:team` prefix. `parseRoute`
 * strips it into `route.team`; `formatRoute` re-emits the prefix only
 * when `route.team` is set. Today (single-team) the app leaves `team`
 * unset, so the prefix is invisible; tomorrow SaaS flips `team` into
 * every navigation and every URL carries the slug without anything
...
interface RouteBase {
   ...
   * `/t/:team/`. Phase 4 leaves this unset by default; SaaS will
   * flip it on globally so every navigation carries team scope.
   */
  team?: string;
}
